...
public class SnakeFinderProcess extends ProcessFile {
    ...
    public boolean process(File in, File out) {
        ...
        try {
            PixM image = PixM.getPixM(new one.empty3.libs.Image(new one.empty3.libs.Image(in)), maxRes);
            for (int i = 0; i < 6; i++) {
            ...
            new one.empty3.libs.Image(1,1,1.saveToFile(image.normalize(0, 1).getImage(), "jpg", out);
            return true;
        } catch (Exception ex) {}

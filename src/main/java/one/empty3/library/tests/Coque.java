...
public class Coque extends RepresentableConteneur {
    ...
    public Coque(Voiture voiture) {
        ...
        add(new Polygon(pointsAccrocheLateraux.getData1d().toArray(gauche), one.empty3.libs.Color.newCol((float)Math.random(),(float)Math.random(),(float)Math.random())));
        add(new Polygon(pointsAccrocheLateraux.getData1d().toArray(droit), one.empty3.libs.Color.newCol((float)Math.random(),(float)Math.random(),(float)Math.random())));
    }
}

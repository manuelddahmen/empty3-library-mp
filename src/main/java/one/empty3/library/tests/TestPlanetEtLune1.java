...
public class TestPlanetEtLune1 extends TestObjetSub {
    static class TextureInvertU extends ImageTexture {
        public TextureInvertU( one.empty3.libs.Image  image) {
            super( image);
        }
    ...
    public static void main(String[] args) {
        ...
        testPlanets.setResolution(Resolution.HD1080RESOLUTION.x(), Resolution.HD1080RESOLUTION.y());
    ...
    public void ginit() throws IOException {
        ...
        sun.texture(new TextureInvertU(new one.empty3.libs.Image(one.empty3.libs.Image.staticLoadFile(sunFilename))));
        earth.texture(new TextureInvertU(new one.empty3.libs.Image(one.empty3.libs.Image.staticLoadFile(earthFilename))));
        moon.texture(new TextureInvertU(new one.empty3.libs.Image(one.empty3.libs.Image.staticLoadFile(moonFilename))));
        ...
        z().texture(new ColorTexture(one.empty3.libs.Color.newCol(0.0f,0.0f,0.0f)));
        scene().texture(new ColorTexture(one.empty3.libs.Color.newCol(0.0f,0.0f,0.0f)));
...
*/
    ...
    public void finit() throws Exception {
        ...
        Point3D[] vecEarth = {earth.getVectX(), earth.getVectY(), earth.getVectZ()};

        Matrix33 matrixB = Matrix33.rotationZ(2.0 * Math.PI * 27.0 / 360.0)
                .mult(new Matrix33(vecEarth).tild());
...
*/
        ...
        vecDirRotate(Point3D.X.mult(100.0), new Point3D(0., 0., 0.), 1.0 * frame() / getMaxFrames(), lX, lY);
        LumierePointSimple lumierePointSimple = new LumierePointSimple(one.empty3.libs.Color.newCol(0.0f,0.0f,0.0f), lX.plus(lY), 1.0);

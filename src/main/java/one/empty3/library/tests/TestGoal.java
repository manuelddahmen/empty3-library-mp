...
public class TestGoal extends TestObjetSub {
    ...
    private Cube cubeA;
    private Cube cubeB;
    private final Color ca = new Color(0xFFC5D3);
    private final Color cb =  one.empty3.libs.Color.newCol(255, 153, 28);


    public void tubeAddPoint(Tubulaire3 tube, Point3D p) {
        boolean add = ((CourbeParametriquePolynomiale) tube.getSoulCurve().getElem()).getCoefficients().getData1d().add(p);
    }
    ...
    public void finit() {
        ...
        scene().add(new LineSegment(pa, pb, new ColorTexture(one.empty3.libs.Color.newCol(0,0,1))));
        ...
        sphere.texture(new ColorTexture(one.empty3.libs.Color.newCol(1.0f,1.0f,1f)));
        scene().add(sphere);

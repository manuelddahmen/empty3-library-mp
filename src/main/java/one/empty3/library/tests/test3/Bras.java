...
public class Bras extends TestObjetSub {
    ...
    @Override
    public void ginit() {
        getZ().couleurDeFond(new TextureCol(one.empty3.libs.Color.newCol(1.0f,1.0f,1f)));
        try {
            textureDefault = new TextureImg(
                    new  one.empty3.libs.Image(
                            Image.staticLoadFile(new File("samples/img/herbe.jpg"))));
        } catch (Exception ex) {
        }

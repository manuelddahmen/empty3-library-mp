...
package one.empty3.library.tests.test3;
...
public final class MainPanel extends JPanel {
    ...
    private static void initBreadcrumbList(Container p, JTree tree) {
        ...
        for (int i = 0; i < paths.length; i++) {
            AbstractButton b = makeButton(tree, copyTreePath(paths, i + 1),  one.empty3.libs.Color.newCol(255, 153, 28));
            p.add(b);
            bg.add(b);
        }
    }
    ...
    private static Component makeBreadcrumbList(List<String> list) {
        ...
        ButtonGroup bg = new ButtonGroup();
        list.forEach(title -> {
            AbstractButton b = makeButton(null, new TreePath(title), new Color(0xFFC5D3));
            p.add(b);
            bg.add(b);
        });
        return p;
...
class ArrowToggleButtonBarCellIcon implements Icon {
    ...
    public void paintIcon(Component c, Graphics g, int x, int y) {
        ...
        Color bgc = (Color) parent.getBackground();
        Color borderColor = (Color) Color.newCol((float) 0.5, (float) 0.5, (float) 0.5).brighter();
        if (c instanceof AbstractButton) {
            ButtonModel m = ((AbstractButton) c).getModel();
            if (m.isSelected() || m.isRollover()) {
                bgc = (Color) c.getBackground();
                borderColor = (Color) Color.newCol((float) 0.5, (float) 0.5, (float) 0.5);
            }
        }
...
class BreadcrumbLayerUI<V extends Component> extends LayerUI<V> {
    ...
    public void paint(Graphics g, JComponent c) {
        ...
        Optional.ofNullable(shape).ifPresent(s -> {
            ...
            g2.setPaint(Color.newCol((float) 0.5, (float) 0.5, (float) 0.5));
            g2.draw(shape);
            g2.dispose();
        });
    }

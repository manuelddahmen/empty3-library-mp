...
package one.empty3.library.tests.tests2.spheres;
 ...
 */
public class TestSphereRotation extends
        TestObjetSub {
    Sphere ts;
    ...
    public void ginit() {
        ...
        for (int i = 0; i < spheres.length; i++) {
            ...
            spheres[i].texture(new TextureCol(one.empty3.libs.Color.newCol((float)Math.random(),(float)Math.random(),(float)Math.random())));
            c.texture(new TextureCol(one.empty3.libs.Color.newCol((float)Math.random(),(float)Math.random(),(float)Math.random())));

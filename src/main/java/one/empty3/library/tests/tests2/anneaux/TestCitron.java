...
package one.empty3.library.tests.tests2.anneaux;
    ...
    public void ginit() {
        ...
        for (int i = 0; i < citrons.length; i++) {
            citrons[i] = new Citron(axe,
                    ...
                    DIM * 4);
            citrons[i].texture(new TextureCol( one.empty3.libs.Color.newCol(255, 153, 28)));
            citrons[i].texture(new TextureImg(   one.empty3.libs.Image.staticLoadFile((new java.io.File("samples/img/herbe.jpg")))));
            scene().add(citrons[i]);
        }
        scene().cameraActive(new Camera(Point3D.Z.mult(DIM * 2), Point3D.O0, Point3D.Y));

        //scene().lumieres().add(new LumierePointSimple(one.empty3.libs.Color.newCol(0,0,1), Point3D.O0, 100));
    }

...
package one.empty3.library.tests.tests2.ver;
...
public class Serpentins2 extends TestObjetSub {
    ...
    public void ginit() {
        for (int i = 0; i < Ns; i++) {
            itext[i] = new TextureImg(     one.empty3.libs.Image.staticLoadFile((new File("c:\\Emptycanvas\\textures\\photo" + (i + 1) + ".jpg"))));
        }
        LumierePonctuelle lumierePonctuelle = new LumierePonctuelle(Point3D.X, one.empty3.libs.Color.newCol((float)Math.random(),(float)Math.random(),(float)Math.random()));
        lumierePonctuelle.setR0(1);

        scene().lumieres().add(lumierePonctuelle);

        lumierePonctuelle = new LumierePonctuelle(Point3D.Y, one.empty3.libs.Color.newCol((float)Math.random(),(float)Math.random(),(float)Math.random()));
        lumierePonctuelle.setR0(1);

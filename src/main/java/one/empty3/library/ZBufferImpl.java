...
public class ZBufferImpl extends Representable implements ZBuffer {
    ...
    public static class IncrementOptimizer {
        ...
        public enum Strategy {
             ...
             * Useful for ensuring quality on large polygons.
             * (Utilize Math.min to cap the increment size)
             */
            ...
            /**
             * Maintains the increment within an interval [min, max].
             */
            CLAMP_WITHIN_RANGE,

            /**
             * No optimization, simply use the calculated increment.
             */
            NONE
        }
                    ...
                    for (double v = n.getStartV(); v + n.getIncrV() <= n.getEndV(); v += n.getIncrV()) {
                        ...
                        if (displayType == SURFACE_DISPLAY_POINTS || displayType == SURFACE_DISPLAY_POINTS_DEEP) {
                            ...
                            if (displayType == SURFACE_DISPLAY_POINTS_DEEP) {
                                ...
                                tracerQuad(
                                        p1, p2, p3, p4, n.texture(), u, u2, v, v2, n);
                            }
    ...
    public void line(Point3D p1, Point3D p2, ITexture t) {
        ...
        Point x2 = camera().coordinatesPoint2D(p2, this);
    ...
    public void itereMaxDist(List<Double> points, ParametricCurve pc, double pStart, double pEnd, ParametricVolume v) {
        ...
        double dist = Point.distance(
                new Point(camera().coordinatesPoint2D(p2start, this)),
                new Point(camera().coordinatesPoint2D(p2End, this)));
        if (dist <= 1.0) {
            points.add(pStart);
            points.add(pEnd);
        } else {
    ...
    protected void tracerAretes(Point3D point3d, Point3D point3d2, Color c) {
        Point p1 = camera().coordinatesPoint2D(point3d, this);
        Point p2 = camera().coordinatesPoint2D(point3d2, this);
    ...
    public void tracerTriangle(Point3D pp1, Point3D pp2, Point3D pp3,
        ...
        for (double a = 0; a < 1.0; a += iteres1) {
            ...
            Point pp = camera().coordinatesPoint2D(p3a, this);
            if (pp != null) {
    ...
    public void tracerQuadInverse(E3Model.FaceWithUv face, Polygon polygonOnImage, Image original, double u, double v) {
        ...
        Point p1, p2, p3, p4;
        p1 = camera().coordinatesPoint2D(polygon.getPoints().getElem(0), this);
        p2 = camera().coordinatesPoint2D(polygon.getPoints().getElem(1), this);
     ...
     */
    public void tracerQuadBak(Point3D pp1, Point3D pp2, Point3D pp3, Point3D pp4, ITexture texture, double u0, double u1,
                           double v0, double v1, ParametricSurface n) {
        Point p1, p2, p3, p4;
        p1 = camera().coordinatesPoint2D(pp1, this);
        p2 = camera().coordinatesPoint2D(pp2, this);
...
*/
    ...
    private void renderIntersectionTriangle(
            FrustumPolygonIntersection.TexturedPoint tp1,
            FrustumPolygonIntersection.TexturedPoint tp2,
            FrustumPolygonIntersection.TexturedPoint tp3,
            ITexture texture,
            ParametricSurface n
    ) {
        Point3D pp1 = tp1.p;
    ...
    private boolean isOccupied(Point3D newValue) {
        ...
        if (newValue != null) {
            Point point = camera().coordinatesPoint2D(newValue, this);
    ...
    public class ImageMap {
        ...
        public boolean testDeep(Point3D p, ITexture texture, double u, double v, Representable representable) {
            try {
                if (texture.getColorAt(u, v) != texture.getTransparent()
                    ...
                    Point point = camera().coordinatesPoint2D(p, that);
        ...
        public void testDeep(Point3D pFinal, Point3D point3D, int colorAt, Representable n) {
            try {
                ...
                Point point = camera().coordinatesPoint2D(pFinal, that);
            } catch (RuntimeException e) {
                e.printStackTrace();
            }
        }
    ...
    public class ImageMapElement {
        ...
        public void dessine(Point3D x3d, int c1) {
            try {
                ...
                Point ce = camera().coordinatesPoint2D(x3d, that);
                if (ce == null) {
                    return;
                }
                // double prof = -1000;

...
public class DistanceProxLinear44_1 extends DistanceBezier2 {
    ...
    double computeTimeMax = 3600*1000;
     ...
     * @param optimizeGrid
     */
    public DistanceProxLinear44_1(List<Point3D> A, List<Point3D> B, List<Point3D> C, Dimension aDimReal, Dimension bDimReal, Dimension cDimReal,
        ...
        while (occ != oldoccc && !ended) {
            ...
            for (int i = 0; i < sizeA; i++) {
                if (iteration == 0) {
                    ...
                    i1_0 = Math.max(0 , Math.min(i1_0, pointAddedA.length-1));
                    i2_0 = Math.max(0 , Math.min(i2_0, pointAddedA[i1_0].length-1));
                    k1_0 = Math.max(0 , Math.min(k1_0, pointAddedC.length-1));
                    k2_0 = Math.max(0 , Math.min(k2_0, pointAddedC[k1_0].length-1));
                    ...
                    j1 = Math.max(0, Math.min(j1, imageAB.length-1));
                    j2 = Math.max(0, Math.min(j2, imageAB[j1].length-1));
                ...
                while (checked < CHECKED_MAX && !candidatesA.isEmpty() && candidatesA.size() - m > 0) {
                    ...
                    i1 = (int) Math.max(0, Math.min(i1, aDimReal.getWidth()-1));
                    i2 = (int) Math.max(0, Math.min(i2, aDimReal.getHeight()-1));
                    j1 = (int) Math.max(0, Math.min(j1, bDimReal.getWidth()-1));
                    j2 = (int) Math.max(0, Math.min(j2, bDimReal.getHeight()-1));
                    k1 = (int) Math.max(0, Math.min(k1, cDimReal.getWidth()-1));
                    k2 = (int) Math.max(0, Math.min(k2, cDimReal.getHeight()-1));
...
//checkedListRight[]
    ...
    private void realSize(List<Point3D> x) {
        for (int i = 0; i < x.size(); i++) {
            x.set(i, x.get(i).multDot(new Point3D(aDimReal.getWidth()-1, aDimReal.getHeight()-1, 0.0)));
        }
    }

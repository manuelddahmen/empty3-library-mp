...
import one.empty3.apps.facedetect.jvm.DistanceProxLinear44
...
class TextureMorphMove2(
    ...
    init {
        try {
            ...
            if(distanceAbClass!=null) {
                editData.distanceABClass = distanceAbClass
                initTexture(editData)
    ...
    fun initTexture(editData: TextureData) {
        if(distanceAbClass==null) {
            throw RuntimeException("distanceAbClass is null")
        } else {
            try {
            ...
            try {
                ...
                if (axPointInB != null) {
                 ...
                 if (editData.distanceAB is DistanceProxLinear43 && (editData.distanceAB as DistanceProxLinear43).jpgRight != null) {
                    ...
                    } else if (editData.distanceAB is DistanceProxLinear44 && (editData.distanceAB as DistanceProxLinear44).jpgRight != null) {
                        val dist4 = editData.distanceAB as  DistanceProxLinear44
                        var c : Point3D = dist4.findAxPointInBa13(u, v);
                            ...
                            val y3 =  Math.max(0, Math.min(c.getY(),  editData.imageFileRight!!.getHeight().toDouble() - 1).toInt())
                            //if(dist4.checkedListC[x3][y3]) {
                    ...
                    } else if (editData.distanceAB !is DistanceProxLinear44) {
...
*/
    ...
    private fun setDistanceAbClass2(pDistanceAbClass: Class<out DistanceAB?>) {
        ...
        if(pDistanceAbClass!=null) {
            ...
            if (pDistanceAbClass==DistanceProxLinear44::class.java && editData.imageFileRight != null)
                editData.convexHull3 =
                    ConvexHull(
                        editData.points3!!.values.stream().toList(),
                        Dimension(
                ...
                try {
                    if (pDistanceAbClass==DistanceProxLinear1::class.java) {
                        ...
                        if (editData.imageFileRight != null) (editData.distanceAB as DistanceProxLinear43).jpgRight =
                            editData.imageFileRight
                    } else if (pDistanceAbClass==DistanceProxLinear44::class.java) {
                        editData.distanceAB = DistanceProxLinear44(
                            lA, lB, lC, aDimReal, bDimReal, cDimReal, editData.opt1, editData.optimizeGrid
                        )
                        (editData.distanceAB as DistanceProxLinear44).setComputeMaxTime(10E15)
                        (editData.distanceAB as DistanceProxLinear44).setJpgRight(editData.imageFileRight)
                        editData.convexHull3 = ConvexHull(
                            lC, cDimReal
                        )
                        if (editData.imageFileRight != null) (editData.distanceAB as DistanceProxLinear44).jpgRight = editData.imageFileRight
...
/*
            ...
            //distanceABclass = distanceAB!!.javaClass
        } else {
            Logger.getAnonymousLogger().log(Level.SEVERE, "distanceMap is null")
        }

...
*/
    ...
    fun processImage(): Image {
        ...
        if (model != null) {
            ...
            if (editData != null) {
                ...
                if (useConstantMeshIncrement) {
                    ...
                    Logger.getAnonymousLogger().info("MinMaxOptimium set "+v+" constant mesh increment :"+useConstantMeshIncrement)
        ...
        if (model != null) {
            ...
            if (service) {
                //   val serviceIntent = Intent(context, MyRenderService::class.java)
    ...
    private fun createTexture() {
        ...
        when (algorithm) {
            ...
            8 -> distanceABClass = DistanceProxLinear44::class.java
    ...
    private fun createNotificationChannel() {
            val name = "Render Results" // User-visible name
            val descriptionText = "Notifications about image rendering" // User-visible description
    }

    private fun sendNotification(success: Boolean) {
        val title = if (success) "Rendering Complete" else "Rendering Failed"
        val message =
            if (success) "The image has been rendered successfully." else "An error occurred during rendering."
        val notificationId = 1 // Use a unique ID (can be incrementing, but manage carefully)

    }

    fun finishing(image: Image) {
            try {
                val renderingSuccessful = image != null
                if (renderingSuccessful) {
                    Logger.getAnonymousLogger().info( "configureFlutterEngine:end  successfully call renderImages1_1")
                } else {
                }
            } catch (e: Exception) {
                Logger.getAnonymousLogger().info("SoundError - Error playing sound: ${e.message}")
            }
    }
}

...
-1;-1; 0; 1; 1; 1];
s.
x = cumsum([data.start(1);
xc(cc)
]);
s.
y = cumsum([data.start(2);
yc(cc)
]);
else
if ~
...
r = 1;
% for
linear interpolation, replicate
one value
else
r = 3;
% for
cubic spline
interpolation, replicate 3
values
        end
if
length(s
...
'Snake has too few control points.')
end
        x = s.x;
x = [x(end - r + 1 :end) ;x;
x(1:r)];
...
s.x(1)],[s.
y;
s.y(1)],'color',[0,0.8,0],'parent',h,'linewidth',1);
set(h,
'nextplot',os);
end
if nargout>0
...
0,'FormatSpacing'),'compact')
disp([inputname(1),' ='])
disp(['dip_snake object with ',
...
return;
        ...
        ii = (0
:N)';    % this is better than (0:N)/N, because of round-off errors.

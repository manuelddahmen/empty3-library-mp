...
public class Mix extends ProcessNFiles {
    ...
    public boolean processFiles(File out, File... ins) {
        ...
        if (ins.length > 1 && ins[0] != null && isImage(ins[0]) && ins[1] != null && isImage(ins[1])) {
            Image i1 = (Image)(new one.empty3.libs.Image(ins[0]));
            Image i2 = (Image)(new one.empty3.libs.Image(ins[1]));
            i1.loadFile(ins[0]);
            i2.loadFile(ins[1]);
            Image read1 = i1;
            Image read2 = i2;
        ...
        if((ins.length==1 && isImage(ins[0])) || catched ) {
            Image image = new one.empty3.libs.Image(ins[0]);
            new one.empty3.libs.Image(ins[0]);
            image.saveFile( out);
            return true;
        }
        return false;

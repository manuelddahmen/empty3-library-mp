...
public class Hist4Contour extends ProcessFile {
    ...
    public boolean process(File in, File out) {
        ...
        //        Circle c2 = getLevel(cc, inP, cc.r/2);
            //new one.empty3.libs.Image(1,1,4).saveToFile(outP.normalize(0, 1).getImage(), "jpg", out);
        outP.getImage().saveFile(out);
            //new one.empty3.libs.Image(1,1,4).saveToFile(outP0.normalize(0, 1).getImage(), "jpg", out);
            return true;
    ...
    private PixM loadIn(File in) {
        PixM inP = null;
        if (maxRes == 0) {
            Image image = new one.empty3.libs.Image(in);
            new one.empty3.libs.Image(in);
            inP = new PixM(image);
        } else {
            Image image = new one.empty3.libs.Image(in);
            new one.empty3.libs.Image(in);

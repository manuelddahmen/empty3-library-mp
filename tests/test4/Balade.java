...
package one.empty3.library.tests.test4;
...
public class Balade extends TestObjetSub {
    ...
    public void ginit() {
        useRecursive = false;
        ITexture ciel_ensoleille = new ColorTexture(one.empty3.libs.Color.newCol(0,0,1));
        ITexture sol_sableux = new ColorTexture(Color.newCol(104, 78, 51));
        try {
            imageTextureTrunk = new ImageTexture(new  one.empty3.libs.Image(Objects.requireNonNull(ImageIO.read(new File("resources/img/CIMG0454-modif-cs4.jpg")))));
            ciel_ensoleille = new ImageTexture(new  one.empty3.libs.Image(Objects.requireNonNull(ImageIO.read(new File("resources/ciel_ensoleille.jpg")))));
            sol_sableux = new ImageTexture(new  one.empty3.libs.Image(Objects.requireNonNull(ImageIO.read(new File("res/img/planets2/others/8k_saturn_ring_alpha.png")))));
        } catch (IOException e) {
        ...
        for (int i = 0; i < 5; i++) {
            ((CourbeParametriquePolynomiale)(polygonSol.getSoulCurve().getElem())).getCoefficients().setElem(Point3D.random(10.0), i);
        }
        ...
        z().scene(scene());
        z().texture(new ColorTexture(one.empty3.libs.Color.newCol(0,0,1)));
